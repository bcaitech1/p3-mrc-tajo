...
def postprocess_qa_predictions(
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        if not version_2_with_negative:
            ...
            # 추론된 answer 후처리 과정. 만약 후처리 후 공백일 경우 후순위 answer를 보낸다. 
            while find_more_span_in_context(predictions[i]["text"], context) == "":
                i += 1
            print('pred_before',predictions[i]["text"]) 
            pred_after = find_more_span_in_context(predictions[i]["text"], context) 
            print('pred_after',pred_after) 
            all_predictions[example["id"]] = pred_after
